...
try:
    ...
    # pyproj is only required if the user wants to use a custom projection
    # other than the default EPSG:4326, Global Spherical Mercator with coordinates in lat/lon
    # error will be raised if the user tries to use a custom projection without pyproj
    pyproj = None

# requests_file is only required if the user wants to use a local file as the tile source
# a local fork of requests_file is included in this package
...
more_headers= {
    ...
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/*,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7'}



requests_session = requests.session()
requests_session.headers.update(more_headers)
